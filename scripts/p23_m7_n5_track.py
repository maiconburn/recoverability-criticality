"""m = -7, n = 5: deep track upward from the kmax-independent root at
B = 2 (0.0784519 - 1.184744i), kmax 4000 with an 8000 gate at 2e-3
(40 digits, residual guard 1e-9), to decide between absorption
(Re -> 0 linearly at finite B_c) and escape (omega B -> c_5 =
0.0696 - 2.4303i). Output: results/p23_m7_n5_track.json.
"""
import json
import pathlib
import sys
...
path = [(mp.mpf("2.0"), mp.mpc("0.0784519", "-1.184744"))]
B, w = path[-1]
step = mp.mpf("0.05")
while B < 6 and step > mp.mpf("1e-4"):
    ...
    if len(path) >= 2:
        ...
        seed = w
    r3 = deep(seed, Bn, 4000)
    if r3 is None:
        step /= 2
        continue
    r6 = deep(r3[0], Bn, 8000)
...
if len(pts) >= 3:
    ...
    Bc, slopes = None, []
pathlib.Path("results/p23_m7_n5_track.json").write_text(json.dumps({"path": pts, "Bc_linear": Bc, "last_slopes": slopes}, indent=1))
