...
mp.mp.dps = 30
M = -4
seed = mp.mpc(0.07574, -1.56106)  # kmax-600 tracker value at B = 0.873 (the later Re-rising segment is a cut-hugging artifact, as in m = -3)
out = {}
for B in (0.9, 1.0, 1.08, 1.14, 1.2, 1.28, 1.4):
    row = {}
    ...
    if a1 and a2 and abs(complex(*a1) - complex(*a2)) < 1e-4:
        ...
        if a2:
            seed = mp.mpc(*a2)
    out[str(B)] = row
    json.dump(out, open("results/p21_m4_n3_deep.json", "w"), indent=1)
