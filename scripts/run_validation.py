...
CANDIDATE_LEVELS = range(2, 16)
DISTANCES = [10.0 ** (-exponent / 2.0) for exponent in range(2, 9)]  # 1e-1 .. 1e-4
WALK_START = -12.0
OUTPUT = Path(__file__).resolve().parent.parent / "results" / "validation.json"
...
def main() -> None:
    ...
    if result is None:
        family_seed = SpectralFamily(
            b_exact, bp_exact, n_factor, collocation_order=SEED_COLLOCATION_ORDER
        )
        walk_seed = track_pair(
            family_seed, [0.0, WALK_START], seed=mirror_seed(family_seed)
        )
        q2_ep, omega_ep, ep_gap, near_pair = find_ep_global(
            exact_geometry, n_factor, walk_seed
        )
    ...
    for order in CANDIDATE_LEVELS:
        ...
        try:
            ...
            for start in (WALK_START, -8.0, -5.0, -3.0):
                ...
                raise RuntimeError(f"no EP found for N={order}")
        errors = {
