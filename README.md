...
- **EP-pair annihilation at λ_ext = 0.1150 ± 0.0005** (shooting-solver arbitration, `results/rho_profile.json`; see `ERRATA.md` E2). The EP exists as a genuine ρ sign-crossing up to λ = 0.1145, grazes zero at λ = 0.1150, and is gone by 0.1155. The two earlier published values (0.1091 in v1.0–v1.1; 0.1248 ≈ 1/8 in v1.2) and the conjectured coincidence with the Konoplya–Zhidenko eikonal threshold are retracted as collocation artifacts — the deep-spacelike region has ~1e10 collocation conditioning, which is precisely why the shooting solver was built; the threshold is now measured with it. The local structure at extinction is a narrow (width < 0.05 in q²) near-zero spike on a broad ρ bowl; its normal form (fold vs level-crossing) is open. The EP survives the entire causality window (λ ≤ 9/100); the halving law holds unchanged at λ = 0.095, 0.105 *inside the acausal band*
