...
A shallow-water tank design that tests the vortex results (count law, damping ladder, core diagnosis, absorption) is in `results/EXPERIMENT_DESIGN_VORTEX_TANK.md`, and P32 is its feasibility audit: the tower's quality factor is an invariant (Q₀ = |m|/2), blind mode counting is impossible (0–1 of 2–12 modes recovered), and the protocol was rewritten around fixed-frequency cluster fitting, which needs per-sample SNR 0.3 at every |m| against 58–514 for individual overtones (`results/RESULTS_P32_TANK_FEASIBILITY.md`). P33 carries the same statement into holography, where the radius of convergence of the gradient expansion is defined: at the mirror exceptional point of the scalar channel the symmetric invariant has a simple zero and the labelled modes a square-root branch point (a loop exchanges them to 1.6e-13 while the invariants return to 1e-13), so the labelled dispersion relation converges only to |q²| = 5.10 while the invariant reaches 7.56, and at a momentum between the two an order-12 series reproduces the invariant to 2.9% while the labelled series errs by 233% (`results/RESULTS_P33_HYDRO_LABELS.md`). P31 takes the same question to field data, the public KW51 railway-bridge monitoring set: the close mode pair at 3.90/4.06 Hz carries anticorrelated tracking error (−0.747 against a +0.54/+0.94 common-mode baseline) and its sum is determined 1.9× better than quadrature, but the cycle also establishes the principle's sharpest limit, that a symmetric *reparameterisation* buys nothing against a full-covariance detector (Hotelling's T² is affine-invariant, verified to 8e-12) and the gains of P29/P30 came from changing the model instead (`results/RESULTS_P31_KW51.md`). The single statement these lines support, with its proof sketch, exponents, eight instances and predictions, is collected in `results/PRINCIPLE_LABELS_PAY.md` (the response is analytic through spectral singularities; only labeled quantities pay).

### Further preregistered lines (P9–P33, plus P31 on field data) — same quarantine level as P8

Beyond P8, the repository carries additional preregistered cycles, each with frozen predictions, kill criteria, and honest verdicts (several died; see `GRAVEYARD.md`): P9 entangled probes (`results/RESULTS_P9_ENTANGLED.md`), P10 Floquet-layer EPs (`results/RESULTS_P10_FLOQUET.md`), P11 wormhole echoes (`results/RESULTS_P11_WORMHOLE.md`), P12 traversable-wormhole/Petz bridge and P13 photon-ring CRB on real EHT coverage (`results/RESULTS_P12_P13.md`, `results/RESULTS_P13_EHT.md`), P14 gravitational-collider dark-matter metrology (`results/RESULTS_P14_GRAVCOLLIDER.md`), P15 EP-N ladder generalization (`results/RESULTS_P15_EPN.md`), the control-neutrality theorem with exact verification (`results/THEOREM_EP_NEUTRALITY.md`), P16 decoherence as a task class, including the past-EP partial coherence protection (`results/RESULTS_P16_DECOHERENCE.md`), and P20 Rayleigh's curse at the EP: the theorem extended to photon counting and to the quantum Fisher information of classical light, with Tsang's super-resolution reproduced in the time domain and shown to need a source non-analytic in the physical unfolding parameter (`results/RESULTS_P20_RAYLEIGH_EP.md`), and P21 the draining-bathtub vortex (rotating acoustic black hole): the counter-rotating overtones that Cardoso–Lemos–Yoshida could not follow are absorbed linearly by the branch cut, the response stays continuous through the absorption (exponent 1.000), no EP exists at real rotation for |m| ≤ 2, and the escaping modes collapse onto the branch point as 1/B, with the escape rule Re c_n(m) > 0 of the pure-vortex tower (`results/RESULTS_P21_VORTEX.md`), and P22 the same arrival in Kerr: the {2,0,9} and {2,−2,13} overtone sequences of Cook–Zalutskiy reach the negative imaginary axis linearly in the spin (no fold), pinned to the polynomial point −9i/4 for m = 0, with a curvature the vortex lacks and with re-emergence from the axis, which the vortex also lacks (`results/RESULTS_P22_KERR_NIA.md`), and P23 the escape count derived: the pure-vortex tower is the light-ring resonance ladder Re c_n = |m|/4 − (3/8)(n+½)²/|m| (the 3/8 from the barrier action J = 2πh + 3πh²), so the number of counter-rotating overtones that survive strong rotation is N(m) = floor(0.8008|m| + ½), confirmed at |m| = 8, 9, 10 (6, 7, 8) with a corrected scaled solver, while the same analytic arrival law predicts Kerr death spins blind to 5e-4 (`results/RESULTS_P23_TOWER_LAW.md`), and P24 the borderline test of the escape rule: the m = −7 overtone whose tower member has Re c = +0.07 escapes continuously (no Kerr-like multiplet in the vortex; the frozen multiplet reading died on its own kill clause after a Muller false negative, `results/RESULTS_P24_VORTEX_MULTIPLET.md`), and P25 the principle on the SXS fits: the labeled amplitude difference of the (2,2,5)/(2,2,6) pair pays the inverse gap (slope −1.10) and, in a controlled gate on one waveform, the symmetric cluster moments cost nothing extra (slope −0.10) while the raw six-spin slope of the symmetric moments (−0.75) is a confounder the freeze failed to control, so two of its three predictions died (`results/RESULTS_P25_SXS_SYMMETRIC.md`), and P26 the vortex-core diagnostic: an absorbing core gives the scale-free 1:3:5 damping ladder, a reflecting core gives a core-radius-dependent spectrum with ultra-narrow trapped modes where the wall-to-light-ring cavity holds a level; the frozen "one trapped mode per radius" and "Neumann equals Dirichlet" clauses died (`results/RESULTS_P26_VORTEX_CORE.md`); P27 the Bohr–Sommerfeld count of those trapped modes, 14/14 on counts including six blind cases (`results/RESULTS_P27_CAVITY_COUNT.md`); and P28 the Kerr analogue of the escape rule, which died: the high-spin m = 0 towers (computed to n = 9, 11, 15 at a = 0.99, beyond the public tables) fade geometrically in the overtone index and lock their damping onto the −i n/4 ladder instead of crossing zero at Cook–Zalutskiy's onsets (`results/RESULTS_P28_KERR_SURVIVORS.md`); and P29 the controlled ringdown comparison: the labeled difference of the crossing pair costs 20–55× the symmetric model's amplitude (slope −0.87 in the gap), while the symmetric moment extracted from the labeled fit costs 5–11× that reference, so the practical recipe is to fit the symmetric model, not to add labeled amplitudes (`results/RESULTS_P29_SXS_CONTROLLED.md`), with the method note on confluent cluster fitting and its regime gap × T_eff ≲ 0.5 (`results/RESULTS_P30_CLUSTER_FIT.md`). None of these is a main result; all are reproducible from `scripts/`.
