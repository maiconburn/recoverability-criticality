...
A shallow-water tank design that tests the vortex results (count law, damping ladder, core diagnosis, absorption) is in `results/EXPERIMENT_DESIGN_VORTEX_TANK.md`, and P32 is its feasibility audit: the tower's quality factor is an invariant (Q₀ = |m|/2), blind mode counting is impossible (0–1 of 2–12 modes recovered), and the protocol was rewritten around fixed-frequency cluster fitting, which needs per-sample SNR 0.3 at every |m| against 58–514 for individual overtones (`results/RESULTS_P32_TANK_FEASIBILITY.md`). The single statement these lines support, with its proof sketch, exponents, eight instances and predictions, is collected in `results/PRINCIPLE_LABELS_PAY.md` (the response is analytic through spectral singularities; only labeled quantities pay).

### Further preregistered lines (P9–P32) — same quarantine level as P8
